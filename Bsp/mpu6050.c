...
float GyroX,GyroY;				//角速度
float AccX,AccY,AccZ;		//加速度
uint32_t time0=0,time1=0;			//存放系统时间，us
...
//	GyroZ = (int16_t)(((int16_t)bufG[4] << 8) + bufG[5]);
	...
	Pitch = (atan(AccX/sqrt(AccY*AccY+AccZ*AccZ)))*57.2958;				//弧度转换为角度 57.2958 = 180/3.1415926
	Pitch_Kalman = Kalman_filter(Pitch, GyroY,(time0-time1)*0.000001 );
	time1 = time0;
//	Pitch_Kalman = FirstOrder(Pitch, GyroX, 0.02);
//	Pitch_Kalman = SecondOrder(Pitch, GyroX, 0.02);
